...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='PyTorch MNIST ResNet Example')
    ...
    if args.dataset == 'imagenet':
        ...
        trainset = torchvision.datasets.ImageFolder(root=traindir, transform=transforms.Compose([
        ...
        testset = torchvision.datasets.ImageFolder(root=testdir, transform=transforms.Compose([
            transforms.Resize(256),
    ...
    if args.arch == 'resnet50':
        ...
        pretrained = False if args.pretrained is None else True
        ...
        pretrained = True
        model = modelarchs.alexnet(
            pretrained=pretrained, block_type=args.block_type)
    ...
    if args.lq:
        ...
        if len(args.bits) == 0:
            ...
            print("Warning: set all quantized bits as {}".format(
                str(args.bits[0])))
            args.bits = [args.bits[0]] * len(target_weights)
        LQ = lqnet.learned_quant(
            target_weights, b=args.bits, needbias=args.needbias)
    ...
    for epoch in range(0, args.epochs):
        ...
        if (acc > bestacc):
            bestacc = acc
            save_state(model, acc, epoch, args, optimizer,
                       True, quantdict, quantActdict)
        else:
            save_state(model, bestacc, epoch, args, optimizer,
                       False, quantdict, quantActdict)
        if args.lq:
            LQ.restoreW()
